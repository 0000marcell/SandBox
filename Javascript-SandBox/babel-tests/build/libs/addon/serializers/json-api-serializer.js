...
var _trim2 = require('lodash/string/trim');
...
var JsonApiSerializer = function () {
  ...
  _createClass(JsonApiSerializer, [{
    ...
    value: function serialize(modelOrCollection) {
      ...
      if (modelOrCollection instanceof _model2.default) {
    ...
    value: function _serializeRelationshipsFor(model, request) {
      ...
      relationshipNames.forEach(function (relationshipName) {
        ...
        if (association instanceof _model2.default) {
    ...
    value: function _getRelationshipNames() {
      ...
      var requestRelationships = (0, _get3.default)(request, 'queryParams.include');
      var relationships = void 0;

      if ((0, _isString3.default)(requestRelationships)) {
        relationships = requestRelationships;
      } else {
        relationships = (0, _get3.default)(this, 'include', []).join(',');
      }

      if (relationships.length) {
        var expandedRelationships = relationships.split(',').map(_trim3.default).map(function (r) {
          return r.split('.').map(function (_, index, elements) {
            return elements.slice(0, index + 1).join('.');
          });
        });

        return (0, _flatten3.default)(expandedRelationships);
      }
      return [];
    }
  }, {
    ...
    value: function _getRelatedWithPath(parentModel, path) {
      return path.split('.').reduce(function (related, relationshipName) {
        return (0, _lodash2.default)(related).map(function (r) {
          return r.reload()[(0, _inflector.camelize)(relationshipName)];
        }).map(function (r) {
...
JsonApiSerializer.extend = _extend2.default;
