...
var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol ? "symbol" : typeof obj; }; // jscs:disable requireParenthesesAroundArrowParam
...
var _schema = require('./orm/schema');
...
var _serializerRegistry = require('./serializer-registry');
...
function createPretender(server) {
  return new _pretender2.default(function () {
    ...
    this.unhandledRequest = function (verb, path) {
      path = decodeURI(path);
      (0, _assert2.default)('Your Ember app tried to ' + verb + ' \'' + path + '\',\n         but there was no route defined to handle this request.\n         Define a route that matches this path in your\n         mirage/config.js file. Did you forget to add your namespace?');
    };
  });
}
...
function extractRouteArguments(args) {
  ...
  if (isOption(lastArg)) {
    lastArg = (0, _assign3.default)({}, defaultRouteOptions, lastArg);
  } else {
    args.push(lastArg);
...
var Server = function () {
  function Server() {
    ...
    this.schema = new _schema2.default(this.db, options.models);
    this.serializerOrRegistry = new _serializerRegistry2.default(this.schema, options.serializers);
  ...
  _createClass(Server, [{
    ...
    value: function passthrough() {
      ...
      if (paths.length === 0) {
        ...
        paths = ['/**', '/'];
      } else if ((0, _isArray3.default)(lastArg)) {
        verbs = paths.pop();
      }
    ...
    value: function loadFixtures() {
      ...
      if (args.length) {
        ...
        fixtures = _pick3.default.apply(undefined, [fixtures].concat(_toConsumableArray(camelizedArgs)));
      }
    ...
    */
    ...
    value: function loadFactories(factoryMap) {
      ...
      (0, _keys3.default)(factoryMap).forEach(function (type) {
        var collectionName = _this2.schema ? (0, _inflector.pluralize)((0, _inflector.camelize)(type)) : (0, _inflector.pluralize)(type);
    ...
    value: function create(type, overrides, collectionFromCreateList) {
      ...
      if (this.schema && this.schema[(0, _inflector.pluralize)((0, _inflector.camelize)(type))]) {
        var modelClass = this.schema[(0, _inflector.pluralize)((0, _inflector.camelize)(type))];
        ...
        if (collectionFromCreateList) {
          ...
          collectionName = this.schema ? (0, _inflector.pluralize)((0, _inflector.camelize)(type)) : (0, _inflector.pluralize)(type);
          collection = this.db[collectionName];
        }

        (0, _assert2.default)(collection, 'You called server.create(' + type + ') but no model or factory was found. Try `ember g mirage-model ' + type + '`.');
    ...
    value: function createList(type, amount, overrides) {
      var list = [];
      var collectionName = this.schema ? (0, _inflector.pluralize)((0, _inflector.camelize)(type)) : (0, _inflector.pluralize)(type);
    ...
    value: function _defineRouteHandlerHelpers() {
      ...
      [['get'], ['post'], ['put'], ['delete', 'del'], ['patch'], ['head']].forEach(function (_ref) {
        ...
        _this3[verb] = function (path) {
          ...
          var _extractRouteArgument2 = _slicedToArray(_extractRouteArgument, 3);

          var rawHandler = _extractRouteArgument2[0];
          var customizedCode = _extractRouteArgument2[1];
          var options = _extractRouteArgument2[2];
    ...
    value: function _registerRouteHandler(verb, path, rawHandler, customizedCode, options) {
      ...
      var routeHandler = new _routeHandler2.default({
        schema: this.schema,
    ...
    value: function _hasModulesOfType(modules, type) {
      ...
      return modulesOfType ? (0, _keys3.default)(modulesOfType).length > 0 : false;
    }
