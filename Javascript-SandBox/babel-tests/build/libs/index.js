...
module.exports = {
  ...
  _lintMirageTree: function _lintMirageTree(mirageTree) {
  ...
  _shouldIncludeFiles: function _shouldIncludeFiles() {
    ...
    if (enabledInProd && explicitExcludeFiles) {
      throw new Error('Mirage was explicitly enabled in production, but its files were excluded ' + 'from the build. Please, use only ENV[\'ember-cli-mirage\'].enabled in ' + 'production environment.');
    }
